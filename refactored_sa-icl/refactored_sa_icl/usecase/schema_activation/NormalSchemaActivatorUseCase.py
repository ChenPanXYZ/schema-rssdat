...
def NormalSchemaActivatorUseCase(
        problem: Problem,
    ...
    refinement_content = [
        {"type": "text", "text": "\nBelow is the **new question** with a preliminary schema\n"}] + problem.to_prompt(including_answer=False) +\
                         [{"type": "text", "text": "Below is the preliminary schema:\n"}, {"type": "text", "text": problem.mental_representation}, {"type": "text", "text": "\nThink about how you could refine the **preliminary schema** based on the example provided below."}] + \
                         knowledges
    system_prompt = _get_schema_solver_prompt(settings)
    messages = [
